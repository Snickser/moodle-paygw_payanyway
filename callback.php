...
if (!$payanywaytx = $DB->get_record('paygw_payanyway', ['id' => $transactionid])) {
    die('FAIL. Not a valid transaction id');
}

if (! $userid = $DB->get_record("user", ["id" => $payanywaytx->userid])) {
...
if (
    isset($data['MNT_ID']) && isset($data['MNT_TRANSACTION_ID']) && isset($data['MNT_OPERATION_ID'])
